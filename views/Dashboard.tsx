...
import { 
  ...
  CheckCircle2, Circle, ArrowRight, Zap, Database, Shield, Terminal, Cpu, Sparkles
} from 'lucide-react';
...
const Dashboard: React.FC<{ onLogout: () => void, onNavigate: (v: AppView) => void }> = ({ onLogout, onNavigate }) => {
  ...
  const [logs] = useState([
    { t: '21:21:43', m: 'Uplink Baileys estabelecido via participantAlt', s: 'success' },
    { t: '21:20:10', m: 'Escaneando checkout abandonado #AF230', s: 'info' },
    { t: '21:18:05', m: 'Agente Neural WayIA carregado (Gemini 3)', s: 'info' }
  ]);
      ...
      <div className="space-y-8 animate-in fade-in duration-700 h-full overflow-y-auto custom-scrollbar pr-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-[#03081a] border border-white/10 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl">
            <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none rotate-12">
               <Cpu size={280} className="text-orange-500" />
            </div>
            <div className="relative z-10">
               <div className="flex items-center gap-3 mb-8">
                  <span className="bg-orange-600/20 text-orange-500 text-[10px] font-black uppercase tracking-[0.3em] px-4 py-2 rounded-full border border-orange-500/20 shadow-lg shadow-orange-600/10">Neural Engine v3.1 Active</span>
                  <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_8px_#10b981]" />
               </div>
               <h1 className="text-5xl font-black text-white tracking-tighter mb-6 italic leading-[0.9] uppercase">Otimização de <br /><span className="text-orange-500">Recuperação 2025</span>.</h1>
               <p className="text-slate-500 text-sm max-w-lg font-medium leading-relaxed mb-10 uppercase tracking-widest">Sua infraestrutura está processando leads em tempo real via Evolution API. A taxa de conversão atual é de 24.2%.</p>
               ...
               <div className="flex flex-wrap gap-4">
                  <button onClick={() => onNavigate(AppView.CHAT_MANAGER)} className="px-10 py-5 bg-orange-600 hover:bg-orange-500 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.3em] shadow-2xl shadow-orange-600/30 transition-all flex items-center gap-4 active:scale-95">
                     Gerenciar Atendimentos <ArrowRight size={20} />
                  </button>
          ...
          <div className="bg-[#020617] border border-white/10 rounded-[3rem] p-8 flex flex-col justify-center space-y-8 shadow-2xl relative overflow-hidden">
             ...
             <div className="flex items-center gap-4 border-b border-white/5 pb-6">
                ...
                <div>
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Status do Servidor</p>
             ...
             <div className="space-y-4">
                <div className="flex justify-between items-center">
                   <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Carga da CPU</span>
                   <span className="text-xs font-black text-white">12%</span>
                </div>
                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                   <div className="h-full bg-orange-600 w-[12%] shadow-[0_0_10px_#ea580c]" />
                </div>
                <div className="flex justify-between items-center pt-2">
                   <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Uptime da API</span>
                   <span className="text-xs font-black text-emerald-500 tracking-widest">99.9% ONLINE</span>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
           ...
           <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-6">
              <StatCard label="Total Recuperado" val={stats.recovered} icon={TrendingUp} color="text-orange-500" />
              <StatCard label="Leads no Funil" val={stats.leads.toString()} icon={Users} color="text-purple-500" />
              <StatCard label="Atendimentos IA" val={stats.chats.toString()} icon={MessageCircle} color="text-blue-500" />
           </div>

           {/* Dashboard Log Area */}
           <div className="lg:col-span-4 bg-[#03081a] border border-white/10 rounded-[2.5rem] p-8 shadow-2xl">
              <div className="flex items-center gap-3 mb-6">
                 <Terminal size={18} className="text-orange-500" />
                 <h4 className="text-[11px] font-black text-white uppercase tracking-widest italic">Logs do Ecossistema</h4>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/5">
                 <button onClick={() => onNavigate(AppView.CHAT_MANAGER)} className="w-full py-4 bg-white/5 rounded-2xl text-[9px] font-black text-slate-400 uppercase tracking-widest border border-white/5 hover:text-white hover:bg-white/10 transition-all">Ver Debug Completo</button>
              </div>
           </div>
        </div>

        <div className="bg-white/[0.01] border border-dashed border-white/10 p-10 rounded-[3rem] text-center">
           <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.6em]">WAYFLOW NEURAL CORE • ENGINE V3.1.2025 • GEMINI PRO POWERED</p>
        </div>
      </div>
    </Layout>
  );
