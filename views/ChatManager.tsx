...
const ChatManager: React.FC<{ onLogout: () => void, onNavigate: (v: AppView) => void }> = ({ onLogout, onNavigate }) => {
  ...
  useEffect(() => {
    const fetchInstances = async () => {
      ...
      try {
        ...
        console.error("Erro ao listar instâncias:", e);
      }
    };
    fetchInstances();
  }, [config]);

  // Sincronização Robusta de Leads
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      // Tenta buscar de 3 fontes diferentes para máxima compatibilidade
      ...
      // 1. Processar conversas ativas (mais importantes)
      ...
      // 2. Mesclar com a lista de contatos (para novos leads)
  ...
  // Sincroniza sempre que selecionar uma instância
  useEffect(() => {
    if (selectedInstanceName) {
      handleSync();
    ...
    try {
      ...
      const formatted: ChatMessage[] = rawMsgs.map((m: any) => {
        const content = m.message?.conversation || 
                        ...
                        m.content ||
                        (m.key?.fromMe ? "Arquivo enviado" : "Mídia recebida");
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setNewMessage(text);
      alert("Erro ao enviar. Verifique a conexão do WhatsApp.");
      ...
      <div className="h-full flex flex-col gap-4 animate-in fade-in">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-[#03081a] border border-white/5 p-4 rounded-2xl shadow-xl shrink-0">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 bg-orange-600/20 rounded-xl flex items-center justify-center text-orange-500 border border-orange-500/20">
                <MessageSquare size={20} />
             </div>
             <h1 className="text-xl font-black text-white italic uppercase tracking-tighter leading-none">Atendimento Online.</h1>
          </div>
          
          <div className="flex items-center gap-3 flex-1 justify-end w-full md:max-w-xl">
             <select 
               value={selectedInstanceName}
               onChange={(e) => setSelectedInstanceName(e.target.value)}
               className="bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-[10px] text-white font-black uppercase tracking-widest outline-none cursor-pointer flex-1"
             >
        ...
        <div className="flex-1 flex gap-4 overflow-hidden min-h-0">
          {/* Listagem de Contatos Lateral */}
          <div className="w-full md:w-80 flex flex-col gap-3 overflow-y-auto custom-scrollbar pr-2 shrink-0">
            {leads.length === 0 && !isSyncing && (
              <div className="py-20 text-center opacity-20 flex flex-col items-center border border-dashed border-white/5 rounded-3xl bg-white/[0.01]">
          ...
          {selectedLead ? (
              ...
              <div className="p-4 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                <div className="flex items-center gap-4">
                   <div className="relative">
                      ...
                      <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-emerald-500 rounded-full border-2 border-[#03081a]" />
                   </div>
                   <div>
                      <h3 className="text-sm font-black text-white italic tracking-tight uppercase leading-none mb-1">{selectedLead.name}</h3>
                      <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Sessão Ativa: {selectedInstanceName}</p>
                   </div>
                </div>
              ...
              <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-6 bg-[radial-gradient(circle_at_top_right,_rgba(245,158,11,0.03),_transparent)]">
                 {messages.length === 0 ? (
                   <div className="h-full flex flex-col items-center justify-center opacity-10">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest">Puxando Mensagens...</p>
                   </div>
                 ) : messages.map(msg => (
            ...
            <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-3xl opacity-20 text-center p-10 bg-white/[0.01]">
               ...
               <h3 className="text-xl font-black text-white italic uppercase tracking-tight mb-2">Central de Mensagens.</h3>
               <p className="text-[10px] font-bold uppercase tracking-widest max-w-xs">Selecione uma conversa ativa ao lado para iniciar o suporte neural.</p>
            </div>
          )}
        </div>
      </div>
