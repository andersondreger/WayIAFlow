...
const ChatManager: React.FC<{ onLogout: () => void, onNavigate: (v: AppView) => void }> = ({ onLogout, onNavigate }) => {
  ...
  const copyFullLog = () => {
    ...
    alert("Log de diagnóstico copiado para suporte!");
  };
  ...
  const performFetch = async (endpoint: string, options: RequestInit = {}) => {
    try {
      ...
      if (res.status === 401) throw new Error("Não Autorizado (API Key inválida no Evolution)");
      if (res.status === 404) throw new Error("Endpoint não encontrado na URL fornecida.");
  ...
  const findDataArray = (obj: any): any[] => {
    ...
    // Profundidade 2
    if (obj.messages && obj.messages.records) return obj.messages.records;
  ...
  const extractRealJid = (m: any): string | null => {
    ...
    // Prioridade total para participantAlt detectado nos seus logs (v2.3.7)
    if (key.participantAlt) return key.participantAlt;
    
    // Se for grupo, ignora ou tenta pegar o remetente real
    if (remote.includes('@g.us')) return key.participant || null;

    // Normalização de LID para JID real
  ...
  const handleSync = useCallback(async () => {
    ...
    setErrorMsg(null);
    setLeads([]);
    addLog(`>>> HANDSHAKE NEURAL: ${selectedInstanceName}`);
    
    try {
      const stateData = await performFetch(`${baseUrl}/instance/connectionState/${selectedInstanceName}`);
      const state = stateData.instance?.state || stateData.state;
      addLog(`Estado de Conexão: ${state}`);

      if (state !== 'open' && state !== 'CONNECTED') throw new Error("Node Offline. Vincule o QR Code primeiro.");

      let leadsMap = new Map<string, KanbanLead>();

      // TENTATIVA: Deep Message Scanning (Ignora o erro de split de chats)
      addLog("Iniciando Deep Scan nas mensagens recentes...");
      const msgData = await performFetch(`${baseUrl}/chat/findMessages/${selectedInstanceName}`, {
        method: 'POST',
        body: JSON.stringify({ where: {}, limit: 120 })
      });
      
      const records = findDataArray(msgData);
      addLog(`Scanner detectou ${records.length} fluxos de dados.`);

      records.forEach((m: any) => {
        const jid = extractRealJid(m);
        // Filtramos para não pegar nossas próprias mensagens enviadas
        if (jid && !jid.includes('@g.us') && !m.key?.fromMe) {
          if (!leadsMap.has(jid)) {
            const phone = jid.split('@')[0];
            let content = "Interação Registrada";
            if (m.message?.conversation) content = m.message.conversation;
            else if (m.message?.videoMessage) content = "[🎥 Vídeo Recebido]";
            else if (m.message?.imageMessage) content = "[📷 Imagem Recebida]";
      ...
      if (finalLeads.length > 0) {
        ...
        addLog("NADA ENCONTRADO: A API não retornou conversas privadas recentes.");
        setErrorMsg("API Conectada, mas o histórico está vazio ou contém apenas mensagens em GRUPOS. O WayFlow filtra apenas chats individuais.");
      }
  ...
  const fetchInstances = useCallback(async () => {
    ...
    } catch (e) { addLog("Impossível contactar cluster Evolution."); }
  ...
  const loadChat = async (lead: KanbanLead) => {
    ...
    const baseUrl = getBaseUrl();
    try {
      addLog(`Sincronizando Histórico: ${lead.phone}`);
    ...
    } catch (e) { addLog("Erro ao baixar mensagens do canal."); }
  };
  ...
  const sendMessage = async () => {
    ...
    } catch (e) { alert("Disparo Falhou."); setNewMessage(text); } finally { setIsSending(false); }
  };
  ...
  return (
    <Layout activeView={AppView.CHAT_MANAGER} onNavigate={onNavigate} onLogout={onLogout}>
      <div className="h-full flex flex-col gap-4 max-h-[calc(100vh-120px)] overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-[#03081a] border border-white/10 p-5 rounded-[2.5rem] shadow-2xl shrink-0">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h1 className="text-xl font-black text-white italic uppercase tracking-tighter leading-none">Console de Atendimento</h1>
                <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest mt-1">Uplink: <span className="text-orange-500">{selectedInstanceName || 'OFFLINE'}</span></p>
             </div>
          </div>
          
          <div className="flex items-center gap-3 w-full md:w-auto">
             <select value={selectedInstanceName} onChange={(e) => setSelectedInstanceName(e.target.value)} className="bg-white/5 border border-white/10 rounded-2xl px-5 py-3.5 text-[10px] text-white font-black uppercase tracking-[0.2em] outline-none cursor-pointer flex-1 md:min-w-[240px]">
             ...
             <button onClick={handleSync} disabled={isSyncing} className="px-8 py-3.5 bg-orange-600 hover:bg-orange-500 disabled:opacity-50 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-3 shadow-xl transition-all active:scale-95">
        ...
        <div className="bg-black/90 border border-white/10 rounded-[1.5rem] p-5 flex flex-col gap-2 overflow-hidden shrink-0 shadow-inner">
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Logs do Motor Neural (v2.3.7 Deep Scan)</span>
                </div>
                <div className="flex gap-2">
                  <button onClick={copyFullLog} className="px-2.5 py-1.5 bg-white/5 rounded-xl text-[8px] font-black text-orange-500 uppercase border border-white/5 hover:bg-white/10 transition-all"><Clipboard size={10} className="inline mr-1"/> Copiar Trace</button>
                  {lastRawResponse && <button onClick={() => setShowRawError(!showRawError)} className="px-2.5 py-1.5 bg-red-500/10 rounded-xl text-[8px] font-black text-red-500 uppercase border border-red-500/10 hover:bg-red-500/20 transition-all"><Microscope size={10} className="inline mr-1"/> Diagnóstico RAW</button>}
                </div>
            </div>
            {showRawError && <div className="mb-2 p-3 bg-red-950/20 border border-red-500/20 rounded-xl text-[9px] font-mono text-red-400 overflow-auto max-h-40 custom-scrollbar"><pre>{JSON.stringify(lastRawResponse, null, 2)}</pre></div>}
            <div className="max-h-24 overflow-y-auto custom-scrollbar font-mono text-[10px] leading-tight space-y-1">
              {debugLog.length === 0 ? <p className="text-slate-700 italic">Inicie a sincronização para monitorar o handshake da API.</p> : debugLog.map((log, i) => <p key={i} className={i === 0 ? 'text-orange-400 font-bold' : 'text-slate-600'}>{log}</p>)}
            </div>
        </div>
        ...
        {errorMsg && (
          <div className="bg-orange-500/10 border border-orange-500/20 p-4 rounded-[1.5rem] flex items-center gap-4 text-orange-500 animate-in slide-in-from-top-2 shadow-lg shrink-0">
             ...
             <div className="flex-1">
               <p className="text-[11px] font-black uppercase tracking-widest leading-none mb-1">Erro de Uplink</p>
               <p className="text-[10px] opacity-80 leading-snug">{errorMsg}</p>
             </div>
             <button onClick={handleSync} className="px-5 py-2.5 bg-orange-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest active:scale-95 transition-all">Retry Scan</button>
          </div>
        )}
        ...
        <div className="flex-1 flex gap-4 overflow-hidden">
          ...
          <div className="w-full md:w-80 flex flex-col gap-3 bg-[#03081a]/50 border border-white/5 rounded-[2.5rem] p-4 overflow-hidden h-full shadow-inner">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pr-1">
              {leads.filter(l => l.name.toLowerCase().includes(searchTerm.toLowerCase())).map(lead => (
                <div key={lead.id} onClick={() => loadChat(lead)} className={`p-4 rounded-3xl border transition-all cursor-pointer flex items-center gap-4 ${selectedLead?.id === lead.id ? 'bg-orange-600/20 border-orange-500/30 shadow-lg shadow-orange-600/5' : 'bg-white/[0.02] border-white/5 hover:border-white/20'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-white font-black text-[11px] truncate uppercase italic tracking-tight mb-1">{lead.name}</h4>
          ...
          {selectedLead ? (
            <div className="flex-1 flex flex-col bg-[#03081a] border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl relative">
              <div className="p-5 border-b border-white/5 flex items-center gap-4 bg-white/[0.02]">
                ...
                <div>
                   ...
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none">+{selectedLead.phone} • Status Online</p>
                </div>
              </div>
              ...
              <div className="p-6 border-t border-white/5 bg-white/[0.01]">
                 <div className="flex items-center gap-4">
                    <input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} placeholder="INJETAR RESPOSTA..." className="flex-1 bg-slate-950 border border-white/10 rounded-2xl py-5 px-7 text-sm text-white outline-none focus:border-orange-500 transition-all font-medium shadow-inner"/>
            ...
            <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-[4rem] opacity-30 text-center p-20 bg-white/[0.01]">
               ...
               <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter mb-4">Command Link Off.</h3>
               <p className="text-[11px] font-bold uppercase tracking-[0.2em] max-w-sm">Use o botão Sincronizar. O Scanner agora ignora identidades LID e foca nos JIDs reais detectados no seu log Evolution v2.3.7.</p>
            </div>
          )}
        </div>
      </div>
