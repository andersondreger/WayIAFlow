...
const ChatManager: React.FC<{ onLogout: () => void, onNavigate: (v: AppView) => void }> = ({ onLogout, onNavigate }) => {
  ...
  const handleSync = useCallback(async () => {
    if (!selectedInstanceName || !config.evoUrl || !config.evoKey) {
      alert("Nenhum canal selecionado ou configurado.");
      return;
    }
    setIsSyncing(true);
    try {
    ...
    try {
      ...
      if (response.ok) {
        setMessages(prev => [...prev, { 
          id: Math.random().toString(), sender: 'agent', content: tempMsg, 
        ...
        <div className="flex items-center justify-between gap-4 bg-[#03081a] border border-white/5 p-5 rounded-2xl shadow-lg shrink-0">
          ...
          <div className="flex items-center gap-3 flex-1 justify-end max-w-2xl">
             ...
             <div className="relative flex-1 max-w-xs">
                ...
                <input placeholder="Procurar lead..." className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-9 pr-4 text-[10px] text-white font-bold outline-none" />
             </div>
            ...
            {leads.map(lead => (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                   ...
                   {lead.unreadCount > 0 && <div className="w-4 h-4 bg-orange-600 rounded-full flex items-center justify-center text-[8px] font-black text-white">{lead.unreadCount}</div>}
                </div>
            ...
            <div className="flex-1 flex flex-col bg-[#03081a] border border-white/5 rounded-3xl overflow-hidden shadow-xl animate-in slide-in-from-right-4">
              <div className="p-4 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <h3 className="text-sm font-black text-white italic tracking-tight uppercase mb-1 leading-none">{selectedLead.name}</h3>
                      <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-2">
                        <div className="w-1 h-1 bg-emerald-500 rounded-full animate-pulse" /> Ativo
                      </span>
                   </div>
                </div>
              ...
              <div className="p-4 bg-white/[0.01] border-t border-white/5">
                 <div className="flex items-center gap-3">
                    <button className="p-3 bg-white/5 rounded-xl text-slate-500 hover:text-white"><Paperclip size={18} /></button>
                    <input 
                      value={newMessage}
